...
class DrawingIssuancePage(ttk.Frame):
    def __init__(self, parent, username="User"):
        ...
        self.row_height = 42
        # Adjusted column widths for Issuance view
        # [ID, Rev, Status, Requested By, Actions]
        self.col_widths = [180, 90, 130, 280, 200]  # last one will stretch
        self.headers = ["Drawing ID", "Revision", "Status", "Requested By", "Actions"]
        self.hover_row = -1
        self.hover_button = -1
        self.dragging_col = -1
        self._build_ui()
    ...
    def _stretch_last_column(self):
        ...
        if canvas_w > fixed + 40:
            self.col_widths[-1] = max(200, canvas_w - fixed)
        else:
            self.col_widths[-1] = 200  # minimum
    ...
    def _handle_issue(self, drawing_no):
        ...
        # Using list comprehension to remove item
    ...
    def _redraw_table(self):
        ...
        for local_idx, d in enumerate(page_data):
            ...
            for col_idx, val in enumerate(values):
                ...
                padx = 12
                # Left align strict text columns, center others
                text_x = x + padx if col_idx in (0,3) else x + w//2
                anchor = "w" if col_idx in (0,3) else "center"
                fg = "#1f2937"
                if col_idx == 3: # Requested By column styling
                    fg = "#4f46e5"
            ...
            # Dynamic button sizing with cap
            btn_width = 85
            # Center buttons in cell
            ...
            for btn_idx, (text, bg, fg_color, cb) in enumerate(buttons):
                ...
                # Darken on hover
                # Simple darkening logic or fixed
                bg_real = bg
                if hovered:
                    # simplistic darken
                    pass # Keep same color but maybe outline changes or assume user likes bold outline
                
                outline = "#1d4ed8" if hovered else "#d1d5db"
                # If hovered, maybe slightly darker bg? 
                # For simplicity, stick to provided bg.
